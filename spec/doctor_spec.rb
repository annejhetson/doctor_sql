...
describe Doctor do
  it 'creates an instance of Doctor' do
    test_doctor = Doctor.new({:doctor_name => "Anne"})
    test_doctor.should be_an_instance_of Doctor
  end

  it 'saves all of the doctors' do
    test_doctor = Doctor.new({:doctor_name => "Anne", :specialty =>
      "Butts"})
    test_doctor.save
  ...
  it 'creates an instance of Doctor and saves it' do
    test_doctor = Doctor.create({:doctor_name => "Liz", :specialty => "Toe Nails"})
    Doctor.all[0].specialty.should eq "Toe Nails"
    Doctor.all[0].id.should be_an_instance_of Fixnum
  end

  it 'deletes a doctor' do
    test_doctor = Doctor.create({:doctor_name => "Anne", :specialty =>
      "Butts"})
    test_doctor2 = Doctor.create({:doctor_name => "Liz", :specialty => "Toe Nails"})
    Doctor.all[1].delete
